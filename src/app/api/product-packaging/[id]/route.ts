...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
    ...
    try {
        ...
        if (error.code === 'P2025') {
            return new NextResponse(JSON.stringify({ message: 'Conditionnement non trouvé.' }), { status: 404 });
        }
        ...
        return new NextResponse(JSON.stringify({ message: 'Une erreur interne est survenue.' }), { status: 500 });
    }
}